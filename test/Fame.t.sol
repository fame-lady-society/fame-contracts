...
contract FameTest is Test {
    ...
    // What happens when multiple nfts are burnt?
    function test_MultiBurn() public {
        address account1 = address(111);
        fame.transfer(account1, 8e24);
        address account2 = address(112);
        vm.prank(account2);
        ...
        vm.expectEmit(address(fame));
        emit DN404.Transfer(account1, account2, 19e23);
        fame.transfer(account2, 19e23);

        address account3 = address(113);
        vm.expectEmit(address(dn404));
        emit DN404Mirror.Transfer(address(0), account3, 8);
