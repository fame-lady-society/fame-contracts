...
- **Send premium revenue directly to a fee recipient** (session-settled: user-directed — chosen over marketplace accrual and configurable fee custody: the marketplace receives one DN404 unit for replenishment and no fee-withdrawal lifecycle is needed). *(2026-08-06: fee recipient is no longer required to be skip-enabled; buyer-identity premium waiver removed.)*
...
- R20. ~~The configured fee recipient must have `getSkipNFT(feeRecipient) == true` when configured and immediately before every premium transfer.~~ **Superseded 2026-08-06:** fee recipient skipNFT is not required. The marketplace must never receive `SKIP_MANAGER` authority or temporarily alter another account's skip posture.
