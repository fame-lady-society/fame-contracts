...
contract DeployBaseUniversalPoolArtMarketplace is Script {
    ...
    function run() external returns (UniversalPoolArtMarketplace market) {
        ...
        _validateStack(fame, creatorMagic, feeRecipient, deployer);
        market = _deployMarketplace(fame, creatorMagic, premium, feeRecipient, owner, deployer);
    }
    ...
    function deployMarketplace(
        ...
        address owner,
        address sender
    ) public returns (UniversalPoolArtMarketplace market) {
        _requireBase();
        _validateStack(fame, creatorMagic, feeRecipient, sender);
        if (owner != sender) revert ConfigurationMismatch("owner", sender, owner);
        market = _deployMarketplace(fame, creatorMagic, premium, feeRecipient, owner, sender);
    }
    ...
    function _deployMarketplace(
        ...
        address owner,
        address sender
    ) internal returns (UniversalPoolArtMarketplace market) {
    ...
    function _validateStack(Fame fame, CreatorArtistMagic creatorMagic, address feeRecipient, address deployer)
        internal
        view
    {
        if (
